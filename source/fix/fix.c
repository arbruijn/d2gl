...
long fixdivquadlong(ulong nl,long nh,ulong d)
{
	Assert(nh < 0 || d < 0 || nh < d);
	return (((long long)nh) << 32 | ((unsigned long long)nl)) / d;
#if 0	
	int i;
	ulong tmp0;
	ubyte tmp1;
...
static inline unsigned int adddiv2(unsigned int a, unsigned int b) { return (a >> 1) + (b >> 1) + (a & b & 1); }
...
int quad_sqrt(uint low,int high)
{
	...
	r = adddiv2(fixdivquadlong(low,high,r),r);
	r = adddiv2(fixdivquadlong(low,high,r),r);
	r = adddiv2(fixdivquadlong(low,high,r),r);
	...
	//t = fixdivquadlong(low,high,r);
	tq.low=tq.high=0;
	fixmulaccum(&tq,old_r,t);
	if (tq.low!=low || tq.high!=high) // remainder != 0
		r++;
