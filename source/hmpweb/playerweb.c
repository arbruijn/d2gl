...
static unsigned int chksum(void *data, int len) {
	unsigned int *p = data, ret = 0;
	len >>= 2;
	while (len--)
		ret += *p++;
	return ret;
...
const char *playerweb_play_data(void *song_data, int song_len,
	...
	mprintf((1, "melo %d=%04x drum %d=%04x\n", melobnk_len, chksum(melobnk_data, melobnk_len),
		drumbnk_len, chksum(drumbnk_data, drumbnk_len)));
...
const char *playerweb_get_data(int n) {
	switch (n) {
		case 0:
			return song_data;
		case 1:
...
void playerweb_set_freq(int freq) {
	player.freq = freq;
	OPL3_Reset(&player.chip, player.freq);
}
