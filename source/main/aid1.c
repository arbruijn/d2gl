...
#if 0
static int	Flinch_scale = 4;
static int	Attack_scale = 24;
#endif
#define	ANIM_RATE		(F1_0/16)
#define	DELTA_ANG_SCALE	16

//static byte Mike_to_matt_xlate[] = {AS_REST, AS_REST, AS_ALERT, AS_ALERT, AS_FLINCH, AS_FIRE, AS_RECOIL, AS_REST};
...
//static int	Num_boss_teleport_segs;
//static short	Boss_teleport_segs[MAX_BOSS_TELEPORT_SEGS];
#ifndef SHAREWARE
//static int	Num_boss_gate_segs;
//static short	Boss_gate_segs[MAX_BOSS_TELEPORT_SEGS];
#endif
...
static byte	Super_boss_gate_list[] = {0, 1, 8, 9, 10, 11, 12, 15, 16, 18, 19, 20, 22, 0, 8, 11, 19, 20, 8, 20, 8};
#define	MAX_GATE_INDEX	( sizeof(Super_boss_gate_list) / sizeof(Super_boss_gate_list[0]) )
#endif

//static int	Ai_info_enabled=0;
//static int	Robot_firing_enabled = 1;
...
static int			Hit_type, Hit_seg;
static fvi_info		Hit_data;
#endif

//static int Num_awareness_events = 0;
//static awareness_event	Awareness_events[MAX_AWARENESS_EVENTS];

//static vms_vector		Believed_player_pos;
...
static byte Ai_transition_table[AI_MAX_EVENT][AI_MAX_STATE][AI_MAX_STATE] = {
	{
...
static int ai_behavior_to_mode(int behavior)
{
	switch (behavior) {
...
void init_ai_object_d1(int objnum, int behavior, int hide_segment)
{
	...
	if (
		#if 0
		(behavior == AIB_HIDE) || (behavior == AIB_FOLLOW_PATH) ||
		#endif
...
// ---------------------------------------------------------------------------------------------------------------------
void init_ai_objects_d1(void)
{
	int	i;

	memset(Ai_local_info, 0, sizeof(Ai_local_info));
	memset(Ai_cloak_info, 0, sizeof(Ai_cloak_info));
	Last_gate_time = Last_teleport_time = Boss_cloak_end_time = 0;
	...
	for (i=0; i<MAX_OBJECTS; i++) {
		...
		if (objp->control_type == CT_AI)
			init_ai_object_d1(i, objp->ctype.ai_info.behavior, objp->ctype.ai_info.hide_segment);
	}
...
#if 0
//-------------------------------------------------------------------------------------------
static void ai_turn_towards_vector(vms_vector *goal_vector, object *objp, fix rate)
{
	vms_vector	new_fvec;
	fix			dot;
...
static int player_is_visible_from_object(object *objp, vms_vector *pos, fix field_of_view, vms_vector *vec_to_player)
{
	fix			dot;
	fvi_query	fq;
...
static void do_ai_robot_hit_attack(object *robot, object *player, vms_vector *collision_point)
{
...
static void make_random_vector(vms_vector *vec)
{
...
static void mprintf_animation_info(object *objp)
{
	...
	mprintf((0, "Next fire = %x, Time = %x\n", ailp->next_fire, ailp->player_awareness_time));
...
//	-------------------------------------------------------------------------------------------------------------------
//static int	Break_on_object = -1;

static void do_firing_stuff_d1(object *obj, int player_visibility, vms_vector *vec_to_player)
{
//mprintf((0, "!"));
...
static void do_ai_robot_hit(object *objp, int type)
{
	if (objp->control_type == CT_AI) {
...
static int		Robot_sound_volume=DEFAULT_ROBOT_SOUND_VOLUME;
...
static void move_towards_segment_center(object *objp)
{
...
static int ai_door_is_openable(object *objp, segment *segp, int sidenum)
{
	int	wall_num;
...
static int create_gated_robot( int segnum, int object_id)
{
	...
	init_ai_object_d1(objp-Objects, default_behavior, -1 );		//	Note, -1 = segment this robot goes to to hide, should probably be something useful
...
static int gate_in_robot(int type, int segnum)
{
	if (segnum < 0)
...
//	----------------------------------------------------------------------
void start_boss_death_sequence_d1(object *objp)
{
...
static int ai_multiplayer_awareness(object *objp, int awareness_level)
{
	int	rval=1;
...
#ifndef NDEBUG
static fix	Prev_boss_shields = -1;
#endif
		...
		if (!object_animates || (ailp->next_fire <= 0)) {
			...
			if (dot >= 7*F1_0/8) {
				...
				if (aip->CURRENT_GUN < Robot_info[obj->id].n_guns) {
					...
					if ( (aip->behavior != AIB_RUN_FROM) && (aip->behavior != AIB_STILL)
						#if 0
...
//-----old, faster way from about 11/06/94-----		aip->GOAL_STATE = AIS_REST;
	...
	if ((ailp->player_awareness_type == PA_WEAPON_ROBOT_COLLISION) || (ailp->player_awareness_type >= PA_PLAYER_COLLISION)) {
		if ((aip->behavior != AIB_STILL) &&
			...
			(aip->behavior != AIB_FOLLOW_PATH) &&
			#endif
			(aip->behavior != AIB_RUN_FROM) && (obj->id != ROBOT_BRAIN))
			ailp->mode = AIM_CHASE_OBJECT;
	}
...
// -- 		mprintf((0, "[%i] ", obj-Objects));
	...
	switch (ailp->mode) {
		case AIM_CHASE_OBJECT: {		// chasing player, sort of, chase if far, back off if close, circle in between
			...
			if ((player_visibility < 2) && (previous_visibility == 2)) { // this is redundant: mk, 01/15/95: && (ailp->mode == AIM_CHASE_OBJECT)) {
				...
				if (!ai_multiplayer_awareness(obj, 53)) {
					if (maybe_ai_do_actual_firing_stuff(obj, aip))
						ai_do_actual_firing_stuff(obj, aip, ailp, robptr, &vec_to_player, dist_to_player, &gun_point, player_visibility, object_animates, -1);
					return;
				}
			...
			if (GameTime - ailp->time_player_seen > CHASE_TIME_LENGTH) {
				...
				do_firing_stuff_d1(obj, player_visibility, &vec_to_player);
			}
			break;
		}
		...
		case AIM_RUN_FROM_OBJECT:
			...
			if (!(Game_mode & GM_MULTI) || player_visibility)
				if (ai_multiplayer_awareness(obj, 75)) {
					ai_follow_path(obj, player_visibility, player_visibility, NULL);
					ai_multi_send_robot_position(objnum, -1);
				}
		...
		case AIM_OPEN_DOOR: {		// trying to open a door.
			...
			move_towards_vector(obj, &goal_vector, 1);
			ai_multi_send_robot_position(objnum, -1);
	...
	if (ailp->player_awareness_type) {
		...
		//if (obj-Objects==102)printf("obj 102 new goal state %d\n", new_goal_state);
...
#if 0
//	-----------------------------------------------------------------------------------
void ai_do_cloak_stuff_d1(void)
{
	int	i;
...
void create_awareness_event_d1(object *objp, int type)
{
...
void init_robots_for_level_d1(void)
{
	Overall_agitation = 0;
}
#endif

int ai_save_state_d1( BFILE * fp )
{
	bfwrite( &Ai_initialized, sizeof(int), 1, fp );
	bfwrite( &Overall_agitation, sizeof(int), 1, fp );
	bfwrite( Ai_local_info, sizeof(ai_local)*MAX_OBJECTS, 1, fp );
	bfwrite( Point_segs, sizeof(point_seg)*MAX_POINT_SEGS, 1, fp );
	bfwrite( Ai_cloak_info, sizeof(ai_cloak_info)*MAX_AI_CLOAK_INFO, 1, fp );
	bfwrite( &Boss_cloak_start_time, sizeof(fix), 1, fp );
	bfwrite( &Boss_cloak_end_time , sizeof(fix), 1, fp );
	bfwrite( &Last_teleport_time , sizeof(fix), 1, fp );
	bfwrite( &Boss_teleport_interval, sizeof(fix), 1, fp );
	bfwrite( &Boss_cloak_interval, sizeof(fix), 1, fp );
	bfwrite( &Boss_cloak_duration, sizeof(fix), 1, fp );
	bfwrite( &Last_gate_time, sizeof(fix), 1, fp );
	bfwrite( &Gate_interval, sizeof(fix), 1, fp );
	bfwrite( &Boss_dying_start_time, sizeof(fix), 1, fp );
	bfwrite( &Boss_dying, sizeof(int), 1, fp );
	bfwrite( &Boss_dying_sound_playing, sizeof(int), 1, fp );
	bfwrite( &Boss_hit_this_frame, sizeof(int), 1, fp );
	bfwrite( &Boss_been_hit, sizeof(int), 1, fp );
	return 1;
}

int ai_restore_state_d1( BFILE * fp )
{
	if (bfread( &Ai_initialized, sizeof(int), 1, fp ) != 1 ||
		bfread( &Overall_agitation, sizeof(int), 1, fp ) != 1 ||
		bfread( Ai_local_info, sizeof(ai_local)*MAX_OBJECTS, 1, fp ) != 1 ||
		bfread( Point_segs, sizeof(point_seg)*MAX_POINT_SEGS, 1, fp ) != 1 ||
		bfread( Ai_cloak_info, sizeof(ai_cloak_info)*MAX_AI_CLOAK_INFO, 1, fp ) != 1 ||
		bfread( &Boss_cloak_start_time, sizeof(fix), 1, fp ) != 1 ||
		bfread( &Boss_cloak_end_time , sizeof(fix), 1, fp ) != 1 ||
		bfread( &Last_teleport_time , sizeof(fix), 1, fp ) != 1 ||
		bfread( &Boss_teleport_interval, sizeof(fix), 1, fp ) != 1 ||
		bfread( &Boss_cloak_interval, sizeof(fix), 1, fp ) != 1 ||
		bfread( &Boss_cloak_duration, sizeof(fix), 1, fp ) != 1 ||
		bfread( &Last_gate_time, sizeof(fix), 1, fp ) != 1 ||
		bfread( &Gate_interval, sizeof(fix), 1, fp ) != 1 ||
		bfread( &Boss_dying_start_time, sizeof(fix), 1, fp ) != 1 ||
		bfread( &Boss_dying, sizeof(int), 1, fp ) != 1 ||
		bfread( &Boss_dying_sound_playing, sizeof(int), 1, fp ) != 1 ||
		bfread( &Boss_hit_this_frame, sizeof(int), 1, fp ) != 1 ||
		bfread( &Boss_been_hit, sizeof(int), 1, fp ) != 1)
		return 0;
	return 1;
}
