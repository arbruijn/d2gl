...
extern int play_sample(int num, int pan, int vol, int repeat);
extern void update_sample(int idx, int pan, int vol);
extern void stop_sample(int idx);
#endif
...
void digi_play_midi_song(char *filename, char *melodic_bank, char *drum_bank, int loop) {
	...
	CFILE *f = NULL;
	if (filename[0] && strlen(filename) < sizeof(altfilename)) {
		strcpy(altfilename, filename);
		altfilename[strlen(altfilename) - 1] = 'q';
		f = cfopen(altfilename, "rb");
		if (f)
